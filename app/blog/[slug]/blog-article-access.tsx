...
export function BlogArticleAccess({
  ...
  useEffect(() => {
    ...
    setMessage(
      displayWelcomeBackMessage(getIdentityDisplayName(storedIdentity)),
    );
    void saveB2BSalesInterest(storedIdentity).catch((saveError) => {
      console.error("B2B Sales briefing interest could not be saved", saveError);
    });
  }, []);
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const unlockedIdentity = await saveB2BSalesInterest({
        email,
        name,
      ...
      {!isUnlocked && (
        <section className="mt-8 rounded-lg border border-[#0A66C2]/20 bg-[#F3F7FD] p-5 sm:p-6">
          ...
          <h2 className="mt-2 text-2xl font-semibold leading-snug text-[#191919]">
            Unlock today's B2B Sales & LinkedIn Daily briefing.
          </h2>
          <p className="mt-3 text-sm leading-6 text-[#666666]">
            Get the complete briefing and help INConnect personalize future
            professional intelligence updates for you.
          </p>
          ...
          {accessState === "checking" ? (
            ...
            <form className="mt-5 grid gap-4" onSubmit={handleSubmit}>
              ...
              <p className="text-xs leading-5 text-[#666666]">
                By unlocking this briefing, you agree that INConnect may store
                your information and B2B Sales & LinkedIn Daily interest.
              </p>
              {error && (
              ...
              <button
                ...
                type="submit"
              >
                {isSubmitting ? "Unlocking..." : "Unlock Briefing"}
              </button>
            </form>
          )}
        </section>
...
async function saveB2BSalesInterest(identity: StoredReturningIdentity) {
